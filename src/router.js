...
module.exports = class Router extends EventEmitter {
  ...
  createRoute(method, path, parent = this, ...callbacks) {
    ...
    for (let path of paths) {
      ...
      if (
        route.use &&
        !needsConversionToRegex(path) &&
        path !== "/*" && // must be predictable path
            ...
            if (optimizedPathToRouter) {
              ...
              setTimeout(() => {
                if (!this.listenCalled) {
                  return; // can only optimize router who's parent is listening
                }
  ...
  _optimizeRoute(route, routes) {
    ...
    for (let i = 0; i < routes.length; i++) {
      ...
      if ((r.pattern instanceof RegExp && r.pattern.test(route.path)) || (typeof r.pattern === "string" && (r.pattern === route.path || r.pattern === "/*"))) {
  ...
  async _routeRequest(req, res, startIndex = 0, routes = this._routes, skipCheck = false, skipUntil) {
    let routeIndex = skipCheck
      ? startIndex
      : findIndexStartingFrom(
          routes,
          (r) => (r.all || r.method === req.method || (r.gettable && req.method === "HEAD")) && this._pathMatches(r, req),
          startIndex,
        );
